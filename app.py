...
# --- Funções de IA ---
def refine_bug_with_gpt(titulo, descricao, comp_atual, comp_ideal, severety, priority, bdd_user_input):
    try:
...
# Corrige coluna errada antiga (severety → severidade)
if 'severety' in bugs_df.columns:
    bugs_df['severidade'] = bugs_df['severety']
    bugs_df.drop(columns=['severety'], inplace=True)

# Corrige coluna errada antiga (priority → prioridade)
if 'priority' in bugs_df.columns:
    bugs_df['prioridade'] = bugs_df['priority']
    bugs_df.drop(columns=['priority'], inplace=True)
...
if 'editing_bug_id' in st.session_state:
    ...
    with st.form("new_bug_form_refined"):
        ...
        severety_refinada = st.selectbox(
            "🚨 Severidade",
            SEVERITY_OPTIONS,
                    ...
                    if analisar_ia:
                        if not titulo or not descricao or not atual:
                            ...
                            with st.spinner("Analisando com IA..."):
                                refined_data = refine_bug_with_gpt(titulo, descricao, atual, ideal, priority, severety, bdd)
                                if refined_data:
    ...
    if not status_bugs.empty:
        with st.expander(f"**{status}** ({len(status_bugs)})"):
            for index, bug in status_bugs.iterrows():
                ...
                # 📅 Data de criação
                data_criacao = bug.get("data_criacao", "")
                if data_criacao:
                    try:
                        data_fmt = datetime.strptime(
                            data_criacao, "%Y-%m-%d %H:%M"
                        ).strftime("%d/%m/%Y %H:%M")
                    except:
                ...
                # 🚨 Severidade
                severidade = bug.get("severidade", "Médio")
                bg_color = SEVERITY_COLORS.get(severidade, "#f5f5f5")

                st.markdown(f"**ID {bug_id}:** {display_title}")
                st.caption(f"Módulo: {bug['modulo']}")

                # 🧾 Resumo visual do bug
                st.markdown(
                    f"""
                    <div style="
                    ...
                    ">
                        📅 <b>Criado:</b> {data_fmt} &nbsp;|&nbsp;
                        🚨 <b>Severidade:</b> {severidade} &nbsp;|&nbsp;
                        📦 <b>Módulo:</b> {bug['modulo']}
                    </div>
                    """,
                    unsafe_allow_html=True
                )
                ...
                with action_cols_1[0]:
                    ...
                    new_status = st.selectbox(
                        ...
                        BUG_STATUS_OPTIONS,
                        index=idx,
                        key=f"status_select_{bug_id}",
                        label_visibility="collapsed"
                    )
                    if new_status != bug['status']:
                        bugs_df.loc[index, 'status'] = new_status
                        save_bugs(project, bugs_df)
                        st.toast(f"Bug {bug_id} movido para '{new_status}'")
                        st.rerun()

                with action_cols_1[1]:
                    st.button(
                        "✏️",
                        key=f"edit_bug_{bug_id}",
                        on_click=start_editing_bug,
                        args=(bug_id,),
                ...
                if bug['status'] in ["Arrumado", "Em Teste", "Reprovado"]:
                    with action_cols_1[2]:
                        st.button(
                            "✅ Validar",
                            key=f"approve_bug_{bug_id}",
                            on_click=set_bug_to_validate,
                            args=(bug.to_dict(), "Aprovado"),
                            help="Gerar Relatório de Aprovação"
                        )
                    with action_cols_1[3]:
                        st.button(
                            "❌ Validar",
                            key=f"reprove_bug_{bug_id}",
                            on_click=set_bug_to_validate,
                            args=(bug.to_dict(), "Reprovado"),
